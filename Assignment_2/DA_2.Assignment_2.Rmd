...
In the first place in order to understand what functional form to include in the regression. We examined lowess regression with highly rated hotels to decide in what functional form to include stars and distance. The figures in the appendix shows that distance and highly rated hotels have negative relationship. distance increases by one mile and. between 1.2 miles and 3 miles highly rated hotels and distance does not indicate in correlation. after 3 miles, highly rated declines. as the graph indicates after 3 stars the relationship between highly rated and stars changes. Price and highly rated has interesting relationship one unit change in price till USD 250 has positive relationship, after it declines.  
...
lpm <-lm(highly_rated ~ lspline(stars, c(3)) + distance, data=data)
...
logit <- glm(highly_rated ~ lspline(stars, c(3)) + distance, data=data, family='binomial')
summary(logit)
glance(logit)
...
library(mfx)
logit_marg <- logitmfx(formula = highly_rated ~ lspline(stars, c(3)) + distance, data=data, atmean=FALSE)
...
# probit coefficients
probit <- glm(highly_rated ~ lspline(stars, c(3)) + distance, data=data, family=binomial(link="probit"))
...
probit_marg <- probitmfx(formula = highly_rated ~ lspline(stars, c(3)) + distance, data=data, atmean=FALSE)
...
```{r}
g1 <- ggplot(data = data, aes(x=price, y=highly_rated)) +
  geom_smooth(method="loess", color="3a5e8cFF") +
  scale_y_continuous(expand = c(0.01,0.01),limits = c(0,1), breaks = seq(0,1,0.2), labels = scales::percent) +
  labs(x = "price",y = "Probability of Highly Rated") +
  theme_light() 
g1
```
