...
```{r echo=FALSE, fig.align = 'left', warning=FALSE, message = FALSE}

# Observations
data[, .N, by = data$gender] %>% kbl()  %>% kable_classic(full_width=F)
```
...
datasummary((`Weekly earnings` = earnwke) + (`Weekly hours worked` = uhours) + w + lnw ~ Mean + SD + Min + Max + Median + P95 + N, data = data )
...
```{r echo=FALSE, warning=FALSE, message = FALSE, fig.height=3, fig.width=3, fig.pos='h'}
